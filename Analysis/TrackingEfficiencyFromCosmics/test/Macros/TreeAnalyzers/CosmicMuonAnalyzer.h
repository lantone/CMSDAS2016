...
CosmicMuonAnalyzer::CosmicMuonAnalyzer(TTree *tree)
{
  ...
  if (tree == 0) {
    ...
    std::cout<<"got TFile f"<<std::endl;
    if (!f) {
      std::cout<<"!f"<<std::endl;
      f = new TFile("");
      std::cout<<"f made into new empty TFile"<<std::endl;
    }
    tree = (TTree*)gDirectory->Get("T");
    std::cout<<"got T and assigned to tree"<<std::endl;

  }
  std::cout<<"end of if tree==0"<<std::endl;
  Init(tree);
  std::cout<<"did Init(tree)"<<std::endl;
}
...
void CosmicMuonAnalyzer::Init(TTree *tree)
{
  ...
  fChain->Add("cosmicMuons1Leg_MC.root");
  //fChain->Add("cosmicMuons1Leg_data.root");
